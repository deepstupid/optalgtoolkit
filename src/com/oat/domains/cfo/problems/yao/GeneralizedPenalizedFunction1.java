...
/**
 * Type: GeneralizedPenalizedFunction1<br/>
 * Date: 09/01/2007<br/>
 * <br/>
 ...
 *
 */
public class GeneralizedPenalizedFunction1 extends CFOProblem
{
    ...
    {
        double p1 = Math.PI / dimensions;
        double p2 = 10 * Math.pow(Math.sin(Math.PI*y(v[0])), 2);
        double p3 = 0.0;
        for (int i = 0; i < dimensions-1; i++)
        {
            p3 += Math.pow(y(v[i]) - 1, 2) * (1 + 10*Math.pow(Math.sin(Math.PI*y(v[i+1])),2)) + Math.pow(y(v[dimensions-1]-1), 2);
        }
        double p4 = 0.0;
