 ...
 * </pre>
 */
public class LangermannFunction2 extends CFOProblem
{
    // 30, 10
    ...
    { 0.806, 0.517, 0.1, 0.908, 0.965, 0.669, 0.524, 0.902, 0.531, 0.876, 0.462, 0.491, 0.463, 0.714, 0.352, 0.869, 0.813, 0.811, 0.828, 0.964, 0.789, 0.360,
    ...
    { 
        ...
        // -sum(c(i)·(exp(-1/pi·sum((x-A(i))^2))·cos(pi·sum((x-A(i))^2)))),
        // i=1:m, 2<=m<=10; 0<=x(i)<=10.

        for (int i = 0; i < m; i++)
        {
            dist = squareDistance(v, a[i], dimensions);
            sum -= c[i] * (Math.exp(-dist / Math.PI) * Math.cos(Math.PI * dist));
        }
        
        return sum;
    }
    
    private final static double squareDistance(double[] x1, double[] x2, int n)
    {
        double dist = 0.0;
        double d = 0.0;
    ...
    {
        return "Langermann's Function 11 (m=4)";
    }
